...
【特殊經歷】
✓ 競賽（如「ICPC亞洲區決賽」「全國資安競賽優勝」）
✓ 研究或業界實績（如「論文發表」「專利申請」「開源貢獻」）
✓ 值得注意的實習（如「Google實習」「竹科IC設計實習」）
✓ 明顯突出的跨域成就（如「醫學×AI聯合專題」）

【格式規範】
- 每個 hashtag 是單一概念，不超過 15 字
- 不在 hashtag 後加括號或補充說明（✗「圖神經網路 (GNN)」→ ✓「圖神經網路」）
- 不用頓號連接多個項目（✗「OSDI、SOSP、NSDI」→ 選最具代表性的一個）
- 要具體，不能只寫類別名（✗「頂會論文」「國際競賽」「實習」→ 寫出具體名稱）
